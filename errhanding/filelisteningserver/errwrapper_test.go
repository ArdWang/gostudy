...
func errUnknown(writer http.ResponseWriter, request *http.Request) error{
	return errors.New("Unknown eror")
}
...
func verifyResponse(response *http.Response, expectCode int, expectMsg string,
	t *testing.T){
	b,_ := ioutil.ReadAll(response.Body)
	body := strings.Trim(string(b),"\n")
	if response.StatusCode != expectCode || body != expectMsg {
		t.Errorf("expect (%d, %s);"+"got (%d, %s)", expectCode,expectMsg,
